...
public class JDBCClient  {
	...
	static String sqlQry = "select fname, lname, mi, street, street2, city, state, zip, homePhone, dayPhone, email, spouse, comments " +
						   "from Acquantance where street != '' order by lname";

	public static ArrayList<Acquantance> createAcqs(String user, String pwd)  {
		
		ArrayList<Acquantance> acqs = new ArrayList();
		try {
			String url = "jdbc:jtds:sqlserver://192.168.10.92:1433/acq_hibernate";
	...
	public static ArrayList<Acquantance> addAcq(ResultSet rset) throws SQLException  {
		ArrayList<Acquantance> acqList = new ArrayList();
		try {
			rset.beforeFirst();
			while (rset.next())  {
				...
				acq.setHomePhone(rset.getString(9));
				acq.setDayPhone(rset.getString(10));
