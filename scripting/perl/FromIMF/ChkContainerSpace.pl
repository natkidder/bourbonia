...
if (scalar(@ARGV) < 3) {
  print "arg1: server, arg2: database, arg3: col name to search,\n, arg4: col value regex to search, arg5: filename prefix\n";
  exit 1;
}
...
my $ColList = <<SELECT;
CName, CDimCode, CNodeCode, CNodeLabel, NodeIndex, NodeLevel, ONamePtr, CNamePtr, EISEditOption, EISPhoHead, EISRefCode
SELECT
my @Cols = split(",",$ColList);
my $SQL = <<EOF;
  SELECT $ColList
FROM tbl_ContainerSpace
WHERE $colWhere like '$valWhere'
ORDER BY CName, NodeIndex
EOF
