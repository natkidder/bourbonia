...
if (scalar(@ARGV) < 3) { ## or $ARGV[0] = "--help") {
  print "ChkCloneProperties.pl: arg1: source country code, arg2: target country code, arg3: filename prefix\n";
  exit 1;
}
...
my $ColList = "n.oname, o.FormulaText, o.LinkedToOname, o.LinkedToDbname";
my @Cols = split(",",$ColList);
my $SQL = <<EOF;
use EDF_BP_Clone
SELECT $ColList 
FROM tbl_ODPS o, tbl_Onames n
WHERE o.oid = n.oid
AND oname like '$targetCode%' and (FormulaText like '%$sourceCode%' or LinkedToOname like '$sourceCode%')
EOF
