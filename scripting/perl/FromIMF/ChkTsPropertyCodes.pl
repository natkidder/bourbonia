...
my $ColList = "n.oname, a.PropertyName, PropertyDescription, Pcode, PLabel, Pdesc";
my @Cols = split(",",$ColList);
my $SQL = <<EOF;
  SELECT $ColList
FROM tbl_ODPS o, tbl_Onames n, tbl_propertycodes p
WHERE o.oid = n.oid and o.Unit = p.pcode
and p.PropertyName = 'Unit' 
AND oname like '449%'
order by pdesc
EOF
