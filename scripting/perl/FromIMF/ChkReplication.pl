...
if (scalar(@ARGV) < 1 or $ARGV[0] eq "--help") {
  print "arg1: filename prefix\n";
  exit 1;
}
...
my $svr = "EDFSWN08P";
my $DB = "EDF_Batch_Prod";
my $FilePref = shift;
my $DSN = "Provider=SQLOLEDB.1;Integrated Security=SSPI;Persist Security Info=False;Initial Catalog=" . $DB . ";Data Source=" . $svr . ";";

my $ColList = "Source_Database_Name, Dest_Database_Name, Enabled, OnDemandOnly";
my @Cols = split(",",$ColList);
my $SQL = <<EOF;
  SELECT $ColList
FROM          dbo.EDF_Replication_Databases
WHERE      Enabled <> 0 AND OnDemandOnly = 0
ORDER BY Source_Database_Name, Dest_Database_Name;
EOF
