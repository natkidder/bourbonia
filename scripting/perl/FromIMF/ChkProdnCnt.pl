...
if (scalar(@ARGV) < 2 or $ARGV[0] eq "--help") {
  print "arg1: IFSM or SRF, arg2: filename prefix\n";
  exit 1;
}
...
my $tbl = "dbo.tblPrintAutomation" . shift;
my $FilePref = shift;
...
my $SQL = <<EOF;
SELECT $ColList
FROM   (SELECT staff_division
        FROM $tbl 
        GROUP BY staff_division) subq
EOF
