...
my $ColList = "a.Ccode, a.Syncid , b.CICOID, b.CCodes, b.UserID, cast(b.CiCoDate as varchar) as CicoDate, b.Status, b.SyncID as sync";
my @Cols = split(",",$ColList);
my $SQL = <<EOF;
SELECT $ColList
FROM tbl_cico a right join tbl_cico_log b on a.syncID = b.syncID
WHERE (b.CCodes like '%921%' or b.Ccodes like '%941%') and status = 'CheckedOut'
