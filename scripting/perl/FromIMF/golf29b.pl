...
my $ColList = "PropertyDescription, b.PropertyName, b.SortID, b.PLevel, Pcode, PLabel, Pdesc, b.PExpression";
my @Cols = split(",",$ColList);
my $SQL = <<EOF;
  SELECT $ColList
FROM tbl_properties a RIGHT JOIN tbl_propertycodes b
ON a.propertyName = b.propertyName
WHERE b.PropertyName like '%EDFCheckOut%' and b.PCode like 'preins' 
EOF
