...
my $ColList = "Dest_DBName, Dest_Server, Source_DBName, Source_Server, cast(restore_date as varchar(20)) as Restore_Date, dbuser, cast(backup_finish_date as varchar(20)) as Finish_Date";
my @Cols = split(",",$ColList);
my $SQL = <<EOF;
  SELECT $ColList
FROM dbo.tbl_EDF_Restore_History 
ORDER BY Dest_DBName, Source_DBName
EOF
