...
if (scalar(@ARGV) < 2) {
  print "arg1: server, arg2: database, arg3: dest file\n";
  exit 1;
}
...
my $ColList = "a.PropertyName, PropertyDescription, Pcode, PLabel, Pdesc";
my @Cols = split(",",$ColList);
my $SQL = <<EOF;
SELECT $ColList
FROM tbl_properties a, tbl_propertycodes b
WHERE a.propertyName = b.propertyName
