...
if (scalar(@ARGV) < 2) {
  print "arg1: server, arg2: database, arg3: filename prefix\n";
  exit 1;
}
...
my $ColList = "FILENAME, SERIES_CODE, SERIES_NUMBER, FIELD_LENGTH, TSCODE, DESCRIPTION";
my @Cols = split(",",$ColList);
my $SQL = <<EOF;
  SELECT $ColList
FROM tbl_ItalyQtyKeyFile
WHERE FILENAME > ' '
ORDER BY FILENAME
EOF
