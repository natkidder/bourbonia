...
/*		modelMap.addAttribute("userid", user.getUserid());
	...
	@RequestMapping(value = "/filePath/post/form", method = RequestMethod.POST)
	public ModelAndView getFile(@ModelAttribute("SpringWeb")FileProperties fileProperties, ModelMap modelMap) {
		System.out.println("1544-2 - comments="+fileProperties.getFileComments());
		modelMap.addAttribute("filePath", fileProperties.getFilePath());
		File file = new File(fileProperties.getFilePath());
		modelMap.addAttribute("exists",file.exists());
		modelMap.addAttribute("fileComments",fileProperties.getFileComments());
		return new ModelAndView("fileResult", "command", fileProperties);
		//return "userResult"; // seems to not work here, though it does in MVC_FormHandling
	}
