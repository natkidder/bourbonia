...
public class SourceMediaType implements UserType {
	/**
	* Indicate whether objects managed by this type are mutable.
	*
	...
	* Reports to Hibernate the number of columns that will be needed to store values managed by this custom type and the SQL types of those columns.
	* We indicate that our type uses a single VARCHAR column.  Though the API specifies that this information be returned as an array.
	*
	* @return a single VARCHAR column.
	*/
	public int[] sqlTypes()  {
		// Allocate a new array each time to protect against Callers changing its contents
	...
	/**
	* Retrieve an instance of the mapped class (ie SourceMedia) from a JDBC {@link ResultSet}.
	*
	* @param rs the results from which the instance should be retrieved.
	* @param name the columns from which the instance should be retrieved.
	...
	*/
	public Object nullSafeGet(ResultSet rs, String[] names, Object owner) throws SQLException {
		...
		// Then find the corresponding enumeration value, using the enumeration's own lookup capability
		try {
	...
	* @param index the parameter index with the prepared statement at which this value is to be written.
	* @throws SQLException if there is a problem accessing the database.
	*/
	public void nullSafeSet(PreparedStatement pst, Object value, int index) throws SQLException {
		String name = null;
		if (value != null) {
			name = ((SourceMedia)value).toString();
		}
		Hibernate.STRING.nullSafeSet(pst, name, index);
	}
	...
	*/
	...
	/**
	* Transform the object into its cacheable representation.  At the very least this method should perform a deep copy if the type
	* is mutable.  That may not be enough for some implementations, however; for example, associations must be cached as identifier
	...
	* entity we are merging.  For immutable object, or null values, it is safe to simply return the first parameter.  For mutable 
	* objects, it is safe to return a copy of the first parameter.  For objects with component values, it might make sense to recursively
