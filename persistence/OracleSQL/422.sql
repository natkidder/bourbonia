/*
illustrating explicit cursor parameters in the for..loop context.  Opposes 422b.sql
*/
set echo off
...
begin
  for v_emp in c_emp(7600,7800) loop
...
column v_emp.empno format a10
column v_emp.job format a15
column v_emp.sal format a10
declare
  cursor c_emp (l_empno in number, h_empno in number) is 
    select * from emp where empno between l_empno and h_empno
    order by empno;
